<script lang="ts">
	...
	import { list, type Collection, update, get } from '$lib/api/collection';
	...
	const fetchCollections = () => {
		const doFn = async () => {
			...
			filteredCollections = filteredCollections.filter((col) => col.id !== collectionId);
	...
	const fetchCollection = async () => {
		const collection = await get(collectionId);
...
<Modal>
	<div class="flex flex-row justify-between items-center">
		<h1 class="text-lg font-bold pb-1">Edit Collection</h1>
	...
	<div class="w-full flex flex-row justify-between content-bottom mt-4">
		...
		<div class="flex space-x-4">
			...
			<Button
				...
				disabled={!valid}
				onClick={() => {
					update(collectionId, name, parentId || null)
						.then(() => {
							good(`Updated collection '${name}'.`);
							onClose();
						})
						.catch(catchBad);
				}}
